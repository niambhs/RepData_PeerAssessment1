...
```{r, echo=TRUE, warning=FALSE, message=FALSE}   
 ...
 for(i in 1:nrow( fulldata)) {
  ...
  weekday_and_weekend_ave <-aggregate(fulldata$steps, by=list(interval,isaweekend), 
                            FUN=mean)
  ...
  require("ggplot2")  
  a <- ggplot(data=weekday_and_weekend_ave, aes(x = interval, y = steps,colour=isaweekend, group=isaweekend))
  a <- a + geom_line() 
